...
export default function TaubatGuide() {
  ...
  const { sinId: paramSinId } = useParams();
  const sinId = searchParams.get('sin') || paramSinId;
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 bg-gradient-to-b from-amber-600 via-amber-700 to-amber-900 dark:from-slate-950 dark:via-amber-950 dark:to-slate-950 text-white z-[100] flex flex-col p-4 sm:p-6 lg:p-8 overflow-y-auto safe-area-top safe-area-bottom"
    >
      {/* Top Header Navbar - Generous & Balanced (Matching SOS Mode) */}
      <header className="max-w-xl mx-auto w-full flex items-center justify-between gap-3 py-2 sm:py-3 shrink-0">
        {/* Return Button */}
        <button
          onClick={() => {
            soundFx.playTap();
        ...
        {/* Center Badge */}
        <div className="flex items-center gap-2 px-3.5 sm:px-4 py-1.5 rounded-full bg-amber-500/20 text-amber-200 border border-amber-500/40 backdrop-blur-md shadow-lg shadow-amber-950/30">
        ...
        {/* Right Actions: Step Indicator + Sound FX Toggle */}
        <div className="flex items-center gap-2">
          <span className="hidden sm:inline-flex text-[11px] font-bold uppercase tracking-wider bg-white/10 px-3 py-1.5 rounded-full border border-white/15 text-amber-200 backdrop-blur-md">
      ...
      {/* Main Content Body */}
      <div className="flex-1 flex flex-col justify-center max-w-xl mx-auto w-full space-y-6 my-auto py-4 sm:py-6">
        ...
        {step === 1 && (
          ...
          >
            <div className="w-16 h-16 rounded-full bg-white/15 border border-white/20 flex items-center justify-center mx-auto shadow-inner">
            ...
            <p className="text-amber-100/90 text-xs sm:text-sm font-serif italic max-w-md mx-auto">
