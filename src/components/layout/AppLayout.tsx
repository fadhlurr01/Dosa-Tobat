...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-slate-950 pb-20 md:pb-0 md:pl-64 flex flex-col font-sans text-[#1F2937] dark:text-slate-200 transition-colors duration-300">
        ...
        <nav className="flex-1 px-3.5 space-y-1.5 mt-2 overflow-y-auto hide-scrollbar">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 relative group',
                  isActive
                    ? item.isAlert
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span className="truncate tracking-wider uppercase font-black text-[11px] leading-none">
                    {item.label}
                  </span>
                  {isActive && (
                    <motion.div
          ...
          {(role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'CONTENT_ADMIN') && (
            <Link
              to="/admin"
              onClick={handleNavClick}
              className="flex items-center gap-3 px-3.5 py-3 rounded-2xl text-xs font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 bg-indigo-50/60 dark:bg-indigo-950/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 hover:shadow-md hover:shadow-indigo-500/10 transition-all mt-4 border border-indigo-200/80 dark:border-indigo-800/40"
            >
      ...
      <header className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 px-3 py-2 sm:px-6 transition-colors shadow-xs">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-1.5 sm:gap-4">
          
          {/* Left: Mobile Brand & Desktop Nav Links with Digital Green Icons */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            {/* Mobile brand header */}
            <div className="flex items-center gap-1.5 md:hidden min-w-0">
              <Link to="/" onClick={handleNavClick} className="flex items-center gap-1.5 font-extrabold text-[#065F46] dark:text-emerald-400 text-xs sm:text-sm group truncate">
                <span className="w-7 h-7 rounded-lg bg-emerald-700 dark:bg-emerald-600 text-white flex items-center justify-center font-black text-xs shadow-sm group-hover:scale-105 transition-transform shrink-0">
                  DT
                </span>
                <span className="truncate font-black tracking-tight">DOSA & TOBAT</span>
              </Link>
            </div>

            {/* Desktop Quick Nav Links with Digital Green Icons */}
            <nav className="hidden md:flex items-center gap-1.5 text-xs font-black">
              <Link
                to="/app"
                onClick={handleNavClick}
                className={cn(
                  "flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all uppercase tracking-wider text-[11px]",
                  location.pathname === '/app' || location.pathname === '/beranda'
                    ? "bg-emerald-100/90 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 font-black shadow-xs"
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <Link
              to="/sos"
              onClick={handleNavClick}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/50 dark:hover:bg-rose-900/60 border border-rose-200 dark:border-rose-900/60 text-rose-700 dark:text-rose-400 text-xs font-bold transition-all active:scale-95 shadow-2xs shrink-0"
              title="Mode Darurat SOS 90-Detik"
            >
              <LifeBuoy className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400 animate-spin-slow shrink-0" />
              <span className="text-[11px] font-extrabold">SOS</span>
              <span className="hidden sm:inline-block text-[9px] font-extrabold px-1 py-0.2 bg-rose-200/80 dark:bg-rose-900 text-rose-800 dark:text-rose-200 rounded">90s</span>
            </Link>

            {/* Quick sound toggle - hidden on mobile to avoid header clutter */}
            <button
              onClick={() => {
                toggleSound();
                soundFx.playTap();
              }}
              className="hidden sm:flex p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors"
              title={soundEnabled ? 'Efek Suara Aktif' : 'Efek Suara Mati'}
            >
              {soundEnabled ? <Volume2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> : <VolumeX className="w-4 h-4 text-slate-400" />}
            </button>

            {/* Quick theme toggle - hidden on mobile to avoid header clutter */}
            <button
              onClick={() => {
                soundFx.playTap();
                setTheme(theme === 'dark' ? 'light' : 'dark');
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto md:max-w-4xl p-3.5 sm:p-6 md:p-8 pb-28 md:pb-8 flex flex-col">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200/80 dark:border-slate-800 px-1 py-1 flex justify-around items-center z-50 safe-area-bottom transition-colors duration-300 shadow-xl">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              cn(
                'flex flex-col items-center gap-0.5 justify-center flex-1 py-1 rounded-xl transition-all duration-200 relative min-w-0',
                isActive
                  ? item.isAlert
            ...
            {({ isActive }) => (
              <>
                <div className={cn(
                  ...
                  <item.icon className={cn("w-4.5 h-4.5", item.isAlert && "text-rose-600 dark:text-rose-400")} />
                </div>
                <span className={cn("text-[8.5px] font-bold uppercase tracking-tight truncate max-w-full px-0.5", item.isAlert && "text-rose-600 dark:text-rose-400")}>
                  {item.label}
                </span>
                {isActive && (
                  <motion.div
                    ...
                    className={cn(
                      "absolute bottom-0 w-5 h-0.5 rounded-full shadow-xs",
